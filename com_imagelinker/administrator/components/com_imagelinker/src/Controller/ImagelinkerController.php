...
class ImagelinkerController extends BaseController
{
    ...
    {
        // Check for request token for security
        ...
        $selectedFolders = isset($jform['folders']) ? (array) $jform['folders'] : [];
        $caseSensitive = isset($jform['case_sensitive']) ? (bool) $jform['case_sensitive'] : false;

        // Call the model's scan method
        $unlinkedImages = $model->scanForUnlinkedImages($selectedFolders, $caseSensitive);

        // Handle form data and user state
        if (is_array($unlinkedImages)) {
            $jform['unlinked_images'] = $unlinkedImages;
            $app->setUserState('com_imagelinker.data', (object) $jform);

            // Display message based on number of unlinked images found
            $unlinkedCount = count($unlinkedImages);
            if ($unlinkedCount > 0) {
                $app->enqueueMessage(Text::sprintf('COM_IMAGELINKER_SCAN_SUCCESS', $unlinkedCount), 'message');
            } else {
                $app->enqueueMessage(Text::_('COM_IMAGELINKER_SCAN_NO_BROKEN'), 'warning');
            }
 ...
 */
  public function delete()
  {
    ...
    if (empty($cid)) {
        $app->enqueueMessage(Text::_('COM_IMAGELINKER_DELETE_ERROR_NO_PATH'), 'error');
        $this->setRedirect(Route::_('index.php?option=com_imagelinker&view=imagelinkerReturn', false));
        return;
    }
    ...
    foreach ($cid as $imagePath) {
        ...
        if ($result === false) {
            $app->enqueueMessage(Text::sprintf('COM_IMAGELINKER_DELETE_ERROR', htmlspecialchars($imagePath, ENT_QUOTES, 'UTF-8')), 'error');
        } elseif ($result > 0) {
            $deletedCount += $result;
        }
    }
    ...
    $currentData = $app->getUserState('com_imagelinker.data', (object) []);
    $unlinkedImages = isset($currentData->unlinked_images) ? (array) $currentData->unlinked_images : [];
    $unlinkedImages = array_values(array_diff($unlinkedImages, $cid));
    $currentData->unlinked_images = $unlinkedImages;
    $app->setUserState('com_imagelinker.data', $currentData);

    // Display message based on deletion result
    if ($deletedCount > 0) {
        $app->enqueueMessage(Text::sprintf('COM_IMAGELINKER_DELETE_SUCCESS', $deletedCount), 'message');
    } else {
        $app->enqueueMessage(Text::_('COM_IMAGELINKER_DELETE_NO_CHANGES'), 'warning');
    }
    ...
    {
        Factory::getApplication()->setUserState('com_imagelinker.data', null);
